...
- **Salary on cards (shipped).** Pay shows on a card when the source actually states it: structured employer comp from Ashby and USAJOBS, plus, new this round, a range parsed straight out of the job description for the many postings that disclose pay in the body under CA/CO/NY/WA pay-transparency laws (Greenhouse, Lever, Ashby, Workable, and Recruitee all carry full text). The parser is deliberately conservative: ranges only, a salary or pay word must sit beside the number, and anything next to bonus, equity, referral, or sign-on wording is rejected, so a sign-on bonus never reads as a salary. Verified against all 26 real Ashby descriptions with zero false positives and against a real referral-bonus posting. Predicted aggregator pay (Adzuna salary_is_predicted estimates, where min equals max) is captured but hidden behind a frontend flag, so a guessed number never poses as a posted salary. Off-switch: env PARSE_SALARY_FROM_TEXT=off. Appears after the next scan repopulates the pool, same path as the freshness labels (pool feed, not the per-user DB-ranked cards yet).
- **Salary and dates on ranked matches (planned, cost-scoped).** The cheap way to fill the gaps the text parser misses is to enrich only the small set of roles the owner-side AI ranks, not all ~125k. The ranker already reads each full description to score it, so it can emit salary, posted date, and close date as extra structured fields in the same call, at marginal token cost and no extra fetch. It must be additive and fail-safe (absent or malformed fields must never break scoring) because it touches the ranking layer, which is the no-touch core; build it isolated and validate exhaustively, and only after the text parser is confirmed clean in production. Date findings from the source data: Ashby exposes publishedAt, but it is the original publish date and stays set for years on evergreen reqs, so wiring it would show a misleading "Posted over a year ago"; left on first_seen "Added X" instead. Greenhouse exposes only updated_at (last edit, not posted). Close dates exist only on USAJOBS (ApplicationCloseDate) and are absent on Greenhouse, Lever, and Ashby; verify the field against a live USAJOBS response before wiring. UI principle for all of this: keep cards minimal, salary as one compact chip, push any extra detail such as a close date into the detail panel, and never clutter the card.
